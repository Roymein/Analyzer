package com.example.d2j.smali;

import com.example.d2j.smali.antlr4.SmaliBaseVisitor;
import com.example.d2j.smali.antlr4.SmaliLexer;
import com.example.d2j.smali.antlr4.SmaliParser;
...
import static com.example.d2j.smali.Utils.*;
...
public class AntlrSmaliUtil {
    public static void acceptFile(SmaliParser.SFileContext ctx, DexFileVisitor dexFileVisitor) {
        ...
        if (sources.size() > 0) {
            dexClassVisitor.visitSource(
                    unescapeStr(sources.get(sources.size() - 1).src.getText())
            );
        }
    ...
    public static void acceptField(SmaliParser.SFieldContext ctx, String className, DexClassVisitor dexClassVisitor) {
        ...
        if (fieldObj.getType() == SmaliLexer.FIELD_FULL) {
            field = parseFieldAndUnescape(fieldObj.getText());
        } else {
            field = parseFieldAndUnescape(className, fieldObj.getText());
        }
        int access = collectAccess(ctx.sAccList());
        Object value = null;

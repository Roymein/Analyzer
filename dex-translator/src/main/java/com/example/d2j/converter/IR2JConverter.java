...
import com.example.d2j.asm.LdcOptimizeAdapter;
import com.example.d2j.dex.Dex2Asm;
...
public class IR2JConverter implements Opcodes {
                ...
                case ASSIGN: {
                    ...
                    switch (v1.vt) {
                        case LOCAL:
                            ...
                            if (v2.vt == Value.VT.LOCAL && (i == ((Local) v2)._ls_index)) {// check for a=a
                                skipOrg = true;
                            } else if (v1.valueType.charAt(0) == 'I') {// check for IINC
                                if (v2.vt == Value.VT.ADD) {
                                    if (isLocalWithIndex(v2.getOp1(), i) && v2.getOp2().vt == Value.VT.CONSTANT) { // a=a+1;
                                    ...
                                    } else if (isLocalWithIndex(v2.getOp2(), i) && v2.getOp1().vt == Value.VT.CONSTANT) { // a=1+a;
        ...
        switch (type.charAt(0)) {
            ...
            case 'L':
                ...
                if (isZeroOrNull(v1) || isZeroOrNull(v2)) { // IF[non]null
                    ...
                    asm.visitJumpInsn(v.vt == Value.VT.EQ ? IFNULL : IFNONNULL, target);
                } else {
                    accept(v1, asm);
                    accept(v2, asm);
                    asm.visitJumpInsn(v.vt == Value.VT.EQ ? IF_ACMPEQ : IF_ACMPNE, target);
                }
                break;
            default:
                // IFx
    ...
    private static void reBuildE1Expression(Value.E1Expr e1, MethodVisitor asm) {
        ...
        switch (e1.vt) {
            ...
            case INSTANCE_OF: {
                TypeExpr te = (TypeExpr) e1;
                asm.visitTypeInsn(e1.vt == Value.VT.CHECK_CAST ? CHECKCAST : INSTANCEOF, toInternal(te.type));
            }
            break;
            case CAST: {
        ...
        if ((e2.vt == Value.VT.ADD || e2.vt == Value.VT.SUB) && e2.op2.vt == Value.VT.CONSTANT) {
            ...
            switch (t.charAt(0)) {
                ...
                case 'I': {
                    ...
                    if (s < 0) {
                        asm.visitLdcInsn(-s);
                        asm.visitInsn(getOpcode(type, e2.vt == Value.VT.ADD ? ISUB : IADD));
                        return;
                    }
                }
                break;

package com.example.d2j.converter;

import com.example.d2j.ir.IrMethod;
import com.example.d2j.ir.Trap;
import com.example.d2j.ir.TypeClass;
...
import com.example.d2j.node.DexCodeNode;
import com.example.d2j.node.TryCatchNode;
import com.example.d2j.node.analysis.DvmFrame;
import com.example.d2j.node.analysis.DvmInterpreter;
import com.example.d2j.reader.Op;

import java.util.*;

import static com.example.d2j.ir.expr.Exprs.*;
import static com.example.d2j.ir.stmt.Stmts.*;
...
public class Dex2IRConverter {
    ...
    public IrMethod convert(boolean isStatic, Method method, DexCodeNode dexCodeNode) {
        ...
        Set<com.example.d2j.ir.expr.Value> phiValues = new HashSet<>();
    ...
    private void addPhi(DvmValue v, Set<com.example.d2j.ir.expr.Value> phiValues, List<AssignStmt> phis) {
        if (v != null) {
            if (v.local != null) {
                if (v.parent != null) {
                ...
                if (phiValues.size() > 0) {
                    phis.add(Stmts.nAssign(v.local, Exprs
                            .nPhi(phiValues.toArray(new com.example.d2j.ir.expr.Value[phiValues.size()]))));
                    phiValues.clear();
                }
            }
        }
    ...
    private DvmInterpreter<DvmValue> buildInterpreter() {
        return new DvmInterpreter<DvmValue>() {
            DvmValue b(com.example.d2j.ir.expr.Value value) {
